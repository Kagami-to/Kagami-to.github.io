...
function fitDetailTitles(root=document){
  DETAIL_TITLE_SELECTORS.forEach(selector=>root.querySelectorAll(selector).forEach(fitDetailTitle));
}

function initDetailTitleFit(){
  fitDetailTitles();
  const observer=new MutationObserver(()=>fitDetailTitles());
  observer.observe(document.body,{childList:true,subtree:true});
  const parents=[...new Set(DETAIL_TITLE_SELECTORS.flatMap(selector=>[...document.querySelectorAll(selector)].map(el=>el.parentElement).filter(Boolean)))];
...
document.addEventListener('DOMContentLoaded',initDetailTitleFit);
