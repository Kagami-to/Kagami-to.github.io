...
function workCardData(w){const en=getLanguage()==='en';return {lines:en?[w.title_en||w.work_id,w.subtitle_en||'']:[w.title_ja||w.work_id,w.title_en||''],imagePath:w.image_path||''}}
...
function characterWorkCard(w){const d=workCardData(w);return characterRelatedCard(`../works/${characterUrl(w.url_id)}.html`,d.lines[0],d.lines[1],'work',[],{imagePath:d.imagePath})}
...
function entityWorkCard(w,h){const d=workCardData(w);return characterRelatedCard(h,d.lines[0],d.lines[1],'work',[],{imagePath:d.imagePath})}
