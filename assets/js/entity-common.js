...
function characterRelatedCard(h,main,sub='',type='work'){const cls=type==='song'?'character-092-song-card':'character-092-work-card';return `<a class="${cls}" href="${escapeHtml(h)}"><span class="character-092-card-icon" aria-hidden="true">${type==='song'?'♪':''}</span><div class="character-092-card-title">${escapeHtml(main)}</div>${sub?`<div class="character-092-card-subtitle">${escapeHtml(sub)}</div>`:''}</a>`}
