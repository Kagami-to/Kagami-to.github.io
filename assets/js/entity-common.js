...
function characterRelatedCard(h,main,sub='',type='work'){const cls=type==='song'?'entity-card entity-card-song':'entity-card entity-card-work';return `<a class="${cls}" href="${escapeHtml(h)}"><span class="entity-card-icon" aria-hidden="true">${type==='song'?'♪':''}</span><div class="entity-card-title">${escapeHtml(main)}</div>${sub?`<div class="entity-card-subtitle">${escapeHtml(sub)}</div>`:''}</a>`}
