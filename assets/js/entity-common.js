...
function characterCardData(c){const en=getLanguage()==='en';return {lines:en?[c.epithet_en||c.epithet_ja||'',c.name_en||c.name_ja||c.character_id,c.name_ja||'']:[c.epithet_ja||c.epithet_en||'',c.name_ja||c.name_en||c.character_id]}}
function workCardData(w){const en=getLanguage()==='en';return {lines:en?[w.title_en||w.title_ja||w.work_id,w.subtitle_en||w.title_ja||'']:[w.title_ja||w.title_en||w.work_id,w.title_en||'']}}
function songCardData(s){const en=getLanguage()==='en';return {lines:en?[s.title_en||s.title_ja||s.song_id,s.title_ja||'']:[s.title_ja||s.title_en||s.song_id]}}
