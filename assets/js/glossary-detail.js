...
async function renderGlossaryDetail(){
  ...
  sections.forEach((section,index)=>{
    const heading=en?section.title_en:section.title_ja;
    const blocks=glossaryContentBlocks(en?section.content_en:section.content_ja);
    if(!heading&&blocks.every(b=>!b.text.trim()))return;
    ...
    blocks.forEach(b=>{if(!b.text.trim())return;const p=document.createElement('div');p.className='glossary-detail-text';p.style.textAlign=b.align;p.innerHTML=glossaryEsc(b.text);el.appendChild(p)});
    if(el.children.length)root.appendChild(el);
  });
